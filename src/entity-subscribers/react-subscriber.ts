...
import { createQueryBuilder, EntitySubscriberInterface, EventSubscriber, InsertEvent, RemoveEvent, SelectQueryBuilder, UpdateEvent } from "typeorm";
...
export class ReactSubscriber implements EntitySubscriberInterface<ReactEntity> {
    ...
    async afterUpdate(event: UpdateEvent<ReactEntity>) {
        ...
        if (entity.targetType === "posts") {
            query = query.update(PostStatistics)
                .where('postId = :postId', { postId: entity.targetId })

        } else {
            query = query.update(CommentStatistics)
